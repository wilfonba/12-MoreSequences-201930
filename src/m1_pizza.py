...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(75.0, 100.0),  # All numbers are approximate.
                rg.Point(50.0, 125.0),
                rg.Point(25.0, 100.0),
                rg.Point(50.0, 75.0)]
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  7 orange dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(300, 150), 125)
    draw_points_on_circle(window, circle, 7, 'orange')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    pizza(window, circle, 100, 'black', 2)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    window = rg.RoseWindow(550, 400, 'Polygon test 3: 12 segments with thin blue lines')
    circle = rg.Circle(rg.Point(250, 200), 100)
    circle.outline_thickness = 3
    polygon(window, circle, 12, 'blue', 3)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('FANCY POLYGON test 4:  51 orange lines on white circle,'
             + ' hops = 15.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    fancy_polygon(window, circle, 51, 15, 'orange', 1)
    window.close_on_mouse_click()
